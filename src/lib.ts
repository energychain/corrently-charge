...
import { Config, BevCondition, TarifDefinition } from "./types";
...
export class CC {
   ...
   */
  getTariffs = async (
    bevCondition: BevCondition
  ): Promise<TarifDefinition[]> => {
    ...
    if (typeof gsiPrediction == "undefined") {
      throw new Error(
        "Invalid GSI Data received:" + JSON.stringify(gsiPredictionAPI.data)
      );
    }
    ...
    for (let i = 0; i < pvPrediction.length; i++) {
      ...
      for (
        let k = j;
        k < gsiPrediction.length && typeof pvPrediction[i].gsi == "undefined";
        k++
      ) {
    ...
    let chargingSessionEnergy =
      bevCondition.capacity * (1 - bevCondition.soc / 100);
    let chargingTime =
      (chargingSessionEnergy / bevCondition.maxpower) * 3600000;
    let ghg = 0;
    let localEnergy = 0;
    ...
    let tarifDefinition: TarifDefinition;
    ...
    while (i < pvPrediction.length && chargingSessionEnergy > 0) {
      if (
        pvPrediction[i].timestamp > startTime - 3600000 &&
    ...
    tarifDefinition = {
      ...
      ghg: Math.round(0),
      priceunit: priceUnits.subscription,
      soctarget: 50,
      localenergy: localEnergy,
    };
